...
每个镜头聚焦**单一动作**，描述要详尽具体。每个镜头时长按照画面内容与对话长度拆解成 5-15 秒的镜头。
...
所有提示词字段必须使用中文，包括 `image_prompt / video_prompt / bgm_prompt / sound_effect`。不要输出英文镜头、画质、风格后缀；不要追加写实、电影感、动漫、高清、摄影质感等固定风格词。项目视觉风格由全局风格提示词统一添加，`image_prompt` 和 `video_prompt` 只描述镜头内容、构图、动作和画面语义。
